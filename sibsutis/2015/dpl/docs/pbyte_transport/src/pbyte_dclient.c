...
static int dclient_proc_error (pb_dclient_t *dclient, int msg_type,
                               dialog_t *dialog, char *data, int size);
...
static char *dclient_info_str (pb_dclient_t *dclient)
{
	...
	pb_identity_to_str(&dclient->identity, 0, identity_str, sizeof (identity_str));
...
static int dclient_identity_generate(identity_t *identity)
{
	time_t t;
	struct timeval tp;
	struct tm *pt;
...
static int dclient_proc_connection (pb_dclient_t *dclient, pb_msg_t *pb_msg)
{
	...
	pb_identity_to_str(&dclient->identity, 1, identity_str, sizeof (identity_str));
