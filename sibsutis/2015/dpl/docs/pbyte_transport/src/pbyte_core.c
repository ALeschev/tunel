...
static int pcore_check_send (pb_core_t *core)
{
	...
	while (--budget > 0)
	{
		...
		pb_log (core, PBYTE_INFO,
		        "%s: Send '%s'",
		        identity_str,
		        pb_msg_type_to_str (pb_msg->message_params.msg_type));
...
static int pcore_recv_identity (pb_core_t *core, zmq_msg_t *msg, identity_t *identity)
{
	int rc, more;
...
static int pcore_check_recv (pb_core_t *core, zmq_msg_t *msg)
{
	int rc, count, budget;
	pb_msg_t *pb_msg = NULL;
	...
	while (--budget > 0)
	{
		...
		if (count < 0) {
			pb_log (core, PBYTE_ERR, "Poll: %s", zmq_strerror(zmq_errno()));
			goto skip_message;
		}
		...
		if (core->mode == ePBYTE_SERVER)
		{
			...
			if (pcore_recv_identity (core, msg, &identity) != 0)
				goto skip_message;
		}
