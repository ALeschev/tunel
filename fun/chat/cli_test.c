...
{
	if (username && strcasecmp(username, "fred") != 0)
		return CLI_ERROR;
	if (password && strcasecmp(password, "nerk") != 0)
		return CLI_ERROR;
	return CLI_OK;
}
...
{
	return (password && !strcasecmp(password, "topsecret"));
}
...
int show_couters(struct cli_def *cli, UNUSED(char *command), char *argv[], int argc)
{
	cli_print(cli, "%s: shit!", __func__);
	return CLI_OK;
}

int show_show(struct cli_def *cli, UNUSED(char *command), char *argv[], int argc)
{
	cli_print(cli, "%s: shit!", __func__);
	return CLI_OK;
}
...
int main()
{
	...
	c = cli_register_command(cli, NULL, "show", show_show, PRIVILEGE_UNPRIVILEGED,
		MODE_EXEC, NULL);

	cli_register_command(cli, c, "counters", show_couters, PRIVILEGE_UNPRIVILEGED,
