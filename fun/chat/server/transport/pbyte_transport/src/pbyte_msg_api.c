...
const char *pb_payload_type_to_str(pb_data_type_t type)
{
	switch(type)
	{
...
{
	const pb_msg_params_t *p_params;
	...
	pb_pb_identity_to_str(&pb_msg->identity, 0, identity_str, sizeof (identity_str));

	pb_logger (logger, lvl, "description:  '%s'", desc? desc:"");
	pb_logger (logger, lvl, "identity:     '%s'", identity_str);
	pb_logger (logger, lvl, "message type: '%s'", pb_pb_msg_type_to_str(p_params->msg_type));
	pb_logger (logger, lvl, "payload type: '%s'", pb_payload_type_to_str(p_params->data_type));
	pb_logger (logger, lvl, "dialog:       '%.*s'", (int)pb_msg->identity.dialog.dialog_len, pb_msg->identity.dialog.dialog_id);
...
int pb_msg_unpack (pb_msg_t **pb_msg, const char *data, int size)
{
	...
	*pb_msg = (pb_msg_t *)calloc(1, sizeof(pb_msg_t));
	...
	p_params->msg_type = (pb_msg_type_t)data[step++];
	if (p_params->msg_type == TRANSFER)
	{
		p_params->data_type = (pb_data_type_t)data[step++];
		memcpy (&tmp_uc, &data[step], sizeof (unsigned char));
		(*pb_msg)->identity.dialog.dialog_len = tmp_uc;
		if ((*pb_msg)->identity.dialog.dialog_len > sizeof((*pb_msg)->identity.dialog.dialog_id))
		{
            free(*pb_msg);
            return -3;
		}
...
{
	int step = 0;
	const pb_dialog_t *p_dialog;
	unsigned dialog_len = 0;
	const pb_msg_params_t *p_params;
	unsigned char tmp_uc;
	...
	dialog_len = p_dialog->dialog_len;
	if (dialog_len > MAX_ALPHANUMS)
	{
		*size = *size - dialog_len + MAX_ALPHANUMS;
		dialog_len = MAX_ALPHANUMS;
	}
	...
	*data = (char *)calloc(1, *size);
	...
	if (p_params->msg_type == TRANSFER)
	{
		...
		tmp_uc = dialog_len;
		memcpy (&(*data)[step], &tmp_uc, sizeof (unsigned char));
		step += sizeof (unsigned char);
		memcpy (&(*data)[step], p_dialog->dialog_id, dialog_len);
		step += dialog_len;
