...
static inline void cell_generate(int initial, int y, int x)
{
    ...
    if (main_field[y][x].view)
    {
        ...
        for (i = 0; i < GENOM_SIZE; i++)
            main_field[y][x].genom[i] = rand()%GEN_MAX;
...
static void proc_gen_none(int y, int x)
{
    // nothing
}

static void proc_gen_up(int y, int x)
{
    if (!y || main_field[y-1][x].view)
        return;

    memcpy(&main_field[y-1][x], &main_field[y][x], sizeof(main_field[y][x]));
    memset(&main_field[y][x], 0, sizeof(main_field[y][x]));

    print_cell(y-1, x);
    // print_cell(y, x);
}

static void proc_gen_down(int y, int x)
{

}

static void proc_gen_left(int y, int x)
{

}

static void proc_gen_right(int y, int x)
{
    ...
    for (y = 0; y < FIELD_HEIGHT; y++)
    {
        for (x = 0; x < FIELD_WIDTH; x++)
        {
            ...
            if (main_field[y][x].view)
            {
                ...
                if (--main_field[y][x].ttl <= 0)
                {
                    cell_die(y, x);
                    continue;
                }

                gen = main_field[y][x].gen];
                if (++main_field[y][x].gen >= GENOM_SIZE)
                    main_field[y][x].gen = 0;

                switch(main_field[y][x].genom[main_field[y][x].gen])
                {
                    case GEN_NONE:
                        proc_gen_none(y, x);
                        break;
...
static void field_dbg_info(int y, int x)
{
    ...
    for (i = 0; i < GENOM_SIZE; i++)
        mvprintw(19 + i%GENOM_SIZE, FIELD_WIDTH + 5, "                   ");
    ...
    // if (main_field[y][x].view)
    {
        ...
        {
            mvprintw(19 + i%GENOM_SIZE, FIELD_WIDTH + 5, "                   ");
            mvprintw(19 + i%GENOM_SIZE, FIELD_WIDTH + 5, "%s %s", genom_str(main_field[y][x].genom[i]),
                     (main_field[y][x].gen == i)? "<---":"");
        }
