 ...
 * Formulae are described on http://en.wikipedia.org/wiki/YUV
 ...
 */
static void YUV422_to_RGB888(int width, int height, unsigned char *src, unsigned char *dst)
{
    int line, column;
 ...
 * programm with EXIT_FAILURE return code.
...
/*
 * Do ioctl and retry if error was EINTR
 * ("A signal was caught during the ioctl() operation.").
 * Parameters are the same as on ioctl.
...
static void imageProcess(const void* p)
{
    ...
    YUV422_to_RGB888(width,height,src,dst);
